...
getsel(void)
{
	...
	for (y = sel.nb.y; y <= sel.ne.y; y++) {
		...
		if ((y < sel.ne.y || lastx >= linelen) &&
		    (!(TLINE(y)[term.col-1].mode & ATTR_WRAP) || sel.type == SEL_RECTANGULAR))
			*ptr++ = '\n';
	}
	*ptr = '\0';
	return str;
...
csihandle(void)
{
	...
	case 'X': /* ECH -- Erase <n> char */
		...
		tclearregion(term.c.x, term.c.y, x, term.c.y);
		break;
...
tdumpline(int n)
{
    ...
    for (i = 0; i < term.col; ++i) {
        if (line[i].u == 0)
            break;
        len += utf8encode(line[i].u, str + len);
    }
    str[len] = '\n';
    str[len+1] = '\0';
    tprinter(str, len+1);
...
void
tloadaltscreen(int a, int b) {}

void
tloaddefscreen(int a, int b) {}
